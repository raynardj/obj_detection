...
class yolo3_loss_on_t(yloss_basic):
    ...
    def forward(self,y_pred,t_box, conf_, cls_, mask, b_box):
        """
...
class yolo3_loss_both(yloss_basic):
    def __init__(self,lbd_coord=1,lbd_obj=5,lbd_noobj=1,lbd_cls=1,testing=False,train_all=True):
        """
        lbd_coord: lambda_coordinate
        lbd_noobj: lambda_no_object
        """
        super(yolo3_loss_both,self).__init__(lbd_coord,lbd_obj,lbd_noobj,lbd_cls,testing,train_all)
    
    def forward(self,y_pred,t_box, conf_, cls_, mask, b_box,only_cls):
        """
        Forward Calculation of loss function
        :param y_pred: bs * grid_width * grid_height * boxes * (4+1+classes)
        :param t_box: bounding box $t$ t is easy for training
...
class yolo3_loss_train_cls(yloss_basic):
    def __init__(self, lbd_coord=1, lbd_obj=5, lbd_noobj=1, lbd_cls=1, testing=False, train_all=True):
        """
        lbd_coord: lambda_coordinate
        lbd_noobj: lambda_no_object
        """
        super(yolo3_loss_train_cls, self).__init__(lbd_coord, lbd_obj, lbd_noobj, lbd_cls, testing, train_all)
    ...
    def forward(self, y_pred, t_box, conf_, cls_, mask, b_box,only_cls):
        ...
        only_cls = only_cls.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).repeat(1,FEAT_W,FEAT_H,BOX).unsqueeze(-1).float()
        
        loss_noobj = self.lbd_noobj * mse(y_pred_conf * mask2_slice * only_cls, conf_ * mask2_slice * only_cls) / 2.0
        loss_obj = mse(y_pred_conf * mask_slice * only_cls, self.lbd_obj * ioumap * mask_slice * only_cls) / 2.0

        loss_x = self.lbd_coord * mse(y_pred_xy[..., 0:1] * mask_slice * only_cls, t_box[..., 0:1] * mask_slice * only_cls) / 2.0
        loss_y = self.lbd_coord * mse(y_pred_xy[..., 1:2] * mask_slice * only_cls, t_box[..., 1:2] * mask_slice * only_cls) / 2.0
        loss_w = self.lbd_coord * mse(y_pred_wh[..., 0:1] * mask_slice * only_cls, t_box[..., 2:3] * mask_slice * only_cls) / 2.0
        loss_h = self.lbd_coord * mse(y_pred_wh[..., 1:2] * mask_slice * only_cls, t_box[..., 3:4] * mask_slice * only_cls) / 2.0
